...
func checkPaths(*EncryptOpts) (string, error) {
	inputPath := os.Args[1]
...
func encrypt(inPath string, outPath string, *EncryptOpts) error {
...
func main() {
	...
	inputPath, err := checkPaths(*EncryptOpts)
	if err != nil {
		log.Fatal(err)
		os.Exit(1)
	}
	...
	outputPath := os.Args[2]
	err = encrypt(inputPath, outputPath, *EncryptOpts)
	if err != nil {
		log.Fatal(err)
		os.Exit(1)
	}
