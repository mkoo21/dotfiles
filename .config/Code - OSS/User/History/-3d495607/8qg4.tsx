...
// params
const COHERENCE = 0.013;
const SEPARATION = 0.05;
const ALIGNMENT = 0.016;
...
const _loop = (camera: THREE.PerspectiveCamera) =>  (boids: Boid[], velocities: Velocity[]) => () => {
    ...
    for( let i = 0; i < NUM_BOIDS; i++ ) {
        ...
        debugger;
        boids[i].rotation.x += TURN_RATE * dx / Math.abs(dx);
        boids[i].rotation.y += TURN_RATE * dy / Math.abs(dy);
        boids[i].rotation.z += TURN_RATE * dz / Math.abs(dz);
